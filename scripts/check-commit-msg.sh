...
: '
  ...
  --ticket-prefixes "DEV ISD" requires "feat(DEV-123): add login page".
...
usage() {
	cat <<EOF
Usage: $(basename "${0}") [--ticket-prefixes "DEV ISD"] <commit-message-file>
...
Examples:
  ...
  $(basename "${0}") --ticket-prefixes "DEV ISD PD" .git/COMMIT_EDITMSG
EOF
}
...
Examples: 'feat(DEV-1234): new form'
          'fix(ISD-4321): fix login issue'
EOF
		exit 1
	fi
else
