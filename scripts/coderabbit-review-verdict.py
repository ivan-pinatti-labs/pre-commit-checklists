...
Ported ahead of need from rsync-crypt's script of the same name, which built
this as the fix for docker-torrent-box-with-vpn's own #114: `CodeRabbit`
reports `success` on three outcomes that are not the same thing. `Review
completed` means a review happened. `Review rate limited` means the quota was
exhausted and nothing read the diff. A skipped draft means CodeRabbit never
looked because it was told not to. Branch protection cannot tell these apart,
because the legacy commit status API it reads has no fifth state for "green
but not for the reason you think", and three of that repository's pull
requests merged with no review ever having happened as a direct result.
`Review Verified` exists to be the context that can tell them apart, so it is
required instead of `CodeRabbit` and grades every one of those outcomes as
`failure` rather than inheriting their `success`. Nothing here is specific to
that repository's incident; the same ambiguity in CodeRabbit's status applies
to any repository using it, which is why this repository carries the fix
before ever needing it itself.
...
required check for no reason. This is not hypothetical: rsync-crypt found and
fixed the identical bug in its own copy of this script, against its own
pull request #32, before this copy was ever ported. Anything else present,
`Review rate limited`, any `Review skipped: ...` description reaching this
lane (a non-bot pull request is never a draft by the time this runs, since
lane 1 already caught that), an error state, or a description this has never
seen before, is `failure`. No exceptions there: this lane is where the
sibling repositories' unreviewed merges happened, and a status this script
does not recognize is exactly the shape a future change to CodeRabbit's
wording would take, so only the two known in-flight strings above move to
`pending`; nothing else gets the benefit of the doubt.
...
# is for. Treating either as `failure` was the bug found on rsync-crypt's #32,
# the first pull request its own copy of this gate ever ran against:
